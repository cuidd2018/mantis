...
public enum URLParamType {
     ...
     * version
     **/
    version("version", MantisConstants.DEFAULT_VERSION),
    /**
     * request timeout
     **/
    ...
    nodeType("nodeType", MantisConstants.NODE_TYPE_SERVICE),

    // 格式为protocol:port
    export("export", ""),
    embed("embed", ""),

    registryRetryPeriod("registryRetryPeriod", 30 * MantisConstants.SECOND_MILLS),
    /* 注册中心不可用节点剔除方式 */
    excise("excise", Excise.excise_dynamic.getName()),
    cluster("cluster", MantisConstants.DEFAULT_VALUE),
    loadbalance("loadbalance", "activeWeight"),
    haStrategy("haStrategy", "failover"),
    protocol("protocol", MantisConstants.PROTOCOL_MOTAN),
    path("path", ""),
    host("host", ""),
    port("port", 0),
    ...
    proxy("proxy", MantisConstants.PROXY_JDK),
    filter("filter", ""),
    ...
    application("application", MantisConstants.FRAMEWORK_NAME),
    module("module", MantisConstants.FRAMEWORK_NAME),
    ...
    check("check", "true"),
    directUrl("directUrl", ""),
    registrySessionTimeout("registrySessionTimeout", MantisConstants.MINUTE_MILLS),
